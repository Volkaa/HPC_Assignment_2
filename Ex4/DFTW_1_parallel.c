	...
	int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
		#pragma omp parallel for
	  	for (int k=0 ; k<N ; k++)
	    {
	    	double private_xr_o = Xr_o[k];
	    	double private_xi_o = Xi_o[k];
	        for (int n=0 ; n<N ; n++)  {
	        	// Real part of X[k]
	            private_xr_o += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
	            // Imaginary part of X[k]
	            private_xi_o += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
