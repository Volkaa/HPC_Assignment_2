...
typedef struct {double xr_o; double xi_o; char padding[128];} update;
...
int main(int argc, char* argv[]){
  ...
  omp_set_num_threads(atoi(argv[1])); // Set the number of thread given in parameter of the program

  // size of input array
  int N = 3000; // 8,000 is a good number for testing
...
int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
  update end [N];
  	...
  	for (int k=0 ; k<N ; k++)
    {
        ...
        for (int n=0 ; n<N ; n++)  {
        	// Real part of X[k]
            end[k].xr_o += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
            // Imaginary part of X[k]
            end[k].xi_o += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
